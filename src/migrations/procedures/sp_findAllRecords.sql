--SELECT * FROM sp_findAllRecords(10,1,'2022-02-27 00:00:01','2022-03-02 23:59:59',null,8,null,null,null,null,null,null,null,null,null);
create or replace FUNCTION sp_findAllRecords(IN param_offset INT, IN param_start INT, IN param_from date,
                                             IN param_to date, IN param_city INT, IN param_state INT,
                                             IN param_record INT, IN param_user INT, IN param_observation INT,
                                             IN param_client INT, IN param_operator INT, IN param_isclient INT,
...
SELECT DISTINCT A."idAddress",
                ...
                A."clientTrackingId",
                A."declaredValue",
                COALESCE(N."description", '')                                                     AS routeObservation,
                COALESCE(AR."txtNote", '')                                                        AS detailObservation,
                COALESCE(NOTE."description", '')                                                  AS record,
                COALESCE(A."comment", '')                                                         AS comment,
                COALESCE(AR."txtNote", '')                                                        AS descriptionRecord,
                COALESCE(Ne."description", '')                                                    AS neighborhood,
                COALESCE(Z."description", '')                                                     AS zone,
                ...
                CASE
                    WHEN (SELECT COUNT(*)
                          FROM public."TBL_MTR_ADDRESS_RECORD" AAR
                          WHERE AAR."idAddress" = A."idAddress"
                            AND AAR."idState" IN (7, 8)) = 0 THEN 1
                    ELSE (SELECT COUNT(*)
                          FROM public."TBL_MTR_ADDRESS_RECORD" AAR
                          WHERE AAR."idAddress" = A."idAddress"
                            AND AAR."idState" IN (7, 8))
                    END                                                                           AS attempt,
                COALESCE((SELECT (SELECT DATE(AAR."date")
                                  FROM public."TBL_MTR_ADDRESS_RECORD" AAR
                                  WHERE AAR."idAddress" = A."idAddress"
                                    AND AAR."idState" IN (8, 9)
                                  ORDER BY AAR."date" DESC
                                  LIMIT 1) -
                                 (SELECT CASE
                                             WHEN EXISTS(SELECT DATE(R2."assignedDate")
                                                         FROM public."TBL_ROUTE" R2
                                                         WHERE R2."idAddress" = A."idAddress"
                                                         ORDER BY R2."assignedDate" ASC
                                                         LIMIT 1)
                                                 THEN (SELECT DATE(R2."assignedDate")
                                                       FROM public."TBL_ROUTE" R2
                                                       WHERE R2."idAddress" = A."idAddress"
                                                       ORDER BY R2."assignedDate" ASC
                                                       LIMIT 1)
                                             ELSE (SELECT DATE(AAR2."date")
                                                   FROM public."TBL_MTR_ADDRESS_RECORD" AAR2
                                                   WHERE AAR2."idAddress" = A."idAddress"
                                                     AND AAR2."idState" = 6
                                                   ORDER BY AAR2."date" ASC
                                                   LIMIT 1)
                                             END)), 0)                                            AS delay,
                CASE
                    WHEN AR."idState" IN (5, 6, 7, 8, 9, 10) THEN CONCAT(U."firstName", ' ', U."lastName")
                    ELSE '' END                                                                   AS courier,
